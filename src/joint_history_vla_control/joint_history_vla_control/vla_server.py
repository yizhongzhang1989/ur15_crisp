...
def vla_predict(url, images, state, instruction, history_action=None):
    """Call external VLA model server with optional history_action."""
    files = []
...
class VLAControlServer:
    ...
    def __init__(self):
        rclpy.init()
        self.node = rclpy.create_node("joint_history_vla_control")
        ...
        # --- VLA config ---
        self._vla_url = "http://10.190.172.212:5500/api/inference"
        ...
        self._last_action = None        # previous prediction for history_action
    ...
    def _find_static_dir(self):
        try:
            ...
            installed = os.path.join(get_package_share_directory("joint_history_vla_control"), "static")
    ...
    def run(self, host="0.0.0.0", port=8092):
        print(f"\n  VLA History Joint Control: http://{host}:{port}\n")
